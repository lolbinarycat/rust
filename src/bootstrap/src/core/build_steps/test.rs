...
fn get_browser_ui_test_version(builder: &Builder<'_>, pnpm: &Path) -> Option<String> {
    get_browser_ui_test_version_inner(builder, pnpm, false)
        .or_else(|| get_browser_ui_test_version_inner(builder, pnpm, true))
}
...
impl Step for RustdocGUI {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        if let Some(ref pnpm) = builder.config.pnpm {
            cmd.arg("--pnpm").arg(pnpm);
        }
...
impl Step for Tidy {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        // pass the path to the pnpm command used for installing js deps.
        if let Some(pnpm) = &builder.config.pnpm {
            cmd.arg(pnpm);
        } else {
            cmd.arg("pnpm");
        }
