...
impl Config {
    pub(crate) fn from_args(args: Vec<String>) -> Self {
        ...
        opts.optopt("", "nodejs", "absolute path of nodejs", "PATH")
            .optopt("", "pnpm", "absolute path of pnpm", "PATH")
        ...
        let Some(pnpm) = matches.opt_str("pnpm").map(PathBuf::from) else {
            eprintln!("`pnpm` was not provided. If not available, please install it");
            std::process::exit(1);
        };
